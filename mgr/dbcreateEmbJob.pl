...
#############################################################################
 ...
 $JOB_LOG =  $DISK1 . $EmbDir . $prodPeriod . "/log/" . $chainDir;
 $JOB_DIR =  "/star/u2e/starreco/" . $prodPeriod ."/requests/". $chainDir; 
 $SUM_DIR =  $DISK1 . $EmbDir . $prodPeriod . "/sum/" . $chainDir;
...
### set run number for embedding jobfiles
...
#      print "Run ID : ", $mrunId, "\n";
    ...
    $jbset = $prodPeriod . "_" . $parts[5] . "_" . $parts[6];
          $mjobFname = $EmbDir . "_" . $jbset ."_". $jfile;
...
##### print $job_set, "\n";
   ...
   my $jb_new = $JobDir . "/new_jobs/" . $EmbDir . "_" . $Jset . "_" . $gfile;
#     print $jb_new, "\n";
  
      my $hpss_raw_dir  = $TOPHPSS_SINK . "/" . $Jsetr;
      my $hpss_raw_file = $inFile;
      my $hpss_dst_dir  = $TOPHPSS_RECO . "/" . $EmbDir . "/" . $Jsetd;
      my $hpss_dst_file0 = $gfile . ".dst.root";
      my $hpss_dst_file1 = $gfile . ".hist.root";
      my $hpss_dst_file2 = $gfile . ".runco.root";
      my $hpss_dst_file3 = $gfile . ".geant.root";
### bfcph.csh shoulf be changed for right link with fz files

      my $executable     = "/afs/rhic/star/packages/" . $jlibVer . "/mgr/bfcph.csh";
      ...
      if (!open (TOM_SCRIPT,">$jb_new")) {printf ("Unable to create job submission script %s\n",$jb_new);}
       ...
       print TOM_SCRIPT "#standard error -- Should be four\n";
