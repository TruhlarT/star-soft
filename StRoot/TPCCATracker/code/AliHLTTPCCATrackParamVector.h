//-*- Mode: C++ -*-
// $Id: AliHLTTPCCATrackParamVector.h,v 1.1 2010/07/26 20:55:38 ikulakov Exp $
// ************************************************************************
...
class AliHLTTPCCATrackParamVector
{
  ...
  public:
    ...
    sfloat_m TransportToX( const sfloat_v &x, const sfloat_v &sinPhi0,
        const sfloat_v &Bz, const float maxSinPhi = .999f, const sfloat_m &mask = sfloat_m( true ) );
...
inline sfloat_m AliHLTTPCCATrackParamVector::TransportToX( const sfloat_v &x, const sfloat_v &sinPhi0,
    const sfloat_v &Bz, const float maxSinPhi, const sfloat_m &_mask )
{
  ...
  const sfloat_v &exi = sfloat_v( Vc::One ) / CAMath::Sqrt( sfloat_v( Vc::One ) - ey * ey ); // RSqrt
...
///mvz end 23.01.2010
  ...
  if ( maxSinPhi > 0.f ) {
    mask &= CAMath::Abs( sinPhi ) <= maxSinPhi;
  }
