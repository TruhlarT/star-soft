// $Id: AliHLTTPCCAGBTracker.cxx,v 1.1 2010/07/26 20:55:38 ikulakov Exp $
// **************************************************************************
...
class ReconstructSliceTracks
{
  ...
  public:
    inline ReconstructSliceTracks( AliHLTArray<AliHLTTPCCATracker> &fSlices_, double *fStatTime_, tbb::spin_mutex &fMutex_ )
        : fSlices( fSlices_ ), fStatTime( fStatTime_ ), fMutex( fMutex_ ) {}
    inline void operator()( const tbb::blocked_range<int> &r ) const {
...
//  GroupHits();
  ...
  AliHLTResizableArray<ClusterData, 1, AliHLTFullyCacheLineAligned> clusterData( fNSlices );
  {
    int offset = 0;
    int nextSlice = 0;
...
//         hitsNew[oldIHit].SetErrZ( curErrZ*wCurZ+oldErrZ*wOldZ );
          // merge labels
        if (perf){
          AliHLTResizableArray<AliHLTTPCCAPerformance::AliHLTTPCCAHitLabel>& hitLabels =  AliHLTTPCCAPerformance::Instance().GetHitLabels();
